...
export default function HeroSection({ copy, shell, theme, status, statusMsg, progress, frameCount, prediction }) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-14 lg:grid-cols-[1fr_0.9fr] lg:items-center lg:py-20">
      <div>
        <p className={`mb-5 inline-flex rounded-md border px-3 py-2 text-xs font-black uppercase tracking-[0.2em] text-[#2f8f83] ${shell.soft}`}>
          {copy.hero.eyebrow}
        </p>
        <h1 className={`max-w-4xl text-5xl font-black leading-[0.95] tracking-tight md:text-7xl ${shell.ink}`}>
          {copy.hero.title}
        </h1>
        <p className={`mt-6 max-w-2xl text-lg leading-8 ${shell.muted}`}>{copy.hero.body}</p>
        ...
        <div className={`mt-10 grid max-w-2xl grid-cols-3 gap-4 border-y py-6 ${isDark ? 'border-[#2e3431]' : 'border-[#ded8cd]'}`}>
      ...
      <div className="relative min-h-[520px] overflow-hidden rounded-md border border-[#2e3431] bg-[#171717] text-white shadow-2xl shadow-[#171717]/20">
        ...
        <div className="relative flex h-full flex-col justify-between p-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black tracking-tight">{copy.console.title}</h2>
            </div>
            <span className={`rounded-md px-3 py-2 text-xs font-black uppercase tracking-[0.16em] ${status === 'recording' ? 'bg-[#ee5c45] text-white' : 'bg-white/10 text-[#d8fff8]'}`}>
              {statusMsg}
            </span>
          </div>

          <div className="my-10 rounded-md border border-white/10 bg-white/[0.06] p-5">
          ...
          <div className="rounded-md bg-white p-5 text-[#171717]">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-[#2f8f83]">{copy.console.latest}</p>
            <div className="mt-3 flex items-end justify-between gap-4">
              <div>
                <h3 className="text-4xl font-black tracking-tight">{prediction?.word || copy.console.awaiting}</h3>
